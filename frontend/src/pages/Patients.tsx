  ...
  async function loadPatients(search = "") {
    ...
    try {
      const { data } = await api.get<Patient[]>("/patients", {
        params: { query: search.trim() || undefined, limit: 100 },
      });
      setPatients(data);
    } catch (err: any) {
      console.error(err);
      setError(err?.response?.data?.detail || "No fue posible cargar los pacientes.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section>
      ...
      <form onSubmit={(event: FormEvent) => { event.preventDefault(); void loadPatients(query); }} className="mt-6 flex flex-col gap-3 rounded-xl border bg-white p-4 shadow-sm sm:flex-row">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre, apellido o teléfono..." className="flex-1 rounded-lg border border-slate-300 px-3 py-2" />
        <button type="submit" className="rounded-lg bg-slate-900 px-5 py-2 font-medium text-white">Buscar</button>
        <button type="button" onClick={() => { setQuery(""); void loadPatients(""); }} className="rounded-lg border border-slate-300 px-5 py-2 font-medium">Limpiar</button>
      </form>
