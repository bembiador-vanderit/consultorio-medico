...
export default function Patients({ onBack, onPatientChanged, onScheduleAppointment, user }: Props) {
  const canAccessClinical = user.roles.includes("doctor") || user.roles.includes("admin");
  ...
  return <section>
    ...
    {canAccessClinical && historyPatient && <ClinicalHistoryPanel patientId={historyPatient.id} patientName={`${historyPatient.first_name} ${historyPatient.last_name}`} onClose={() => setHistoryPatient(null)} />}
