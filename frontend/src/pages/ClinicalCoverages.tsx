...
type Substitute = { id: number; full_name: string };
type Coverage = { id: number; principal_doctor_id: number; principal_doctor_name: string; substitute_doctor_id: number; substitute_doctor_name: string; center_id: number; center_name: string; starts_at: string; ends_at: string; revoked_at: string | null; status: "active" | "future" | "expired" | "revoked"; created_at: string };

const statusLabels = { active: "Vigente", future: "Programada", expired: "Expirada", revoked: "Revocada" };
...
export default function ClinicalCoverages({ user, onBack }: Props) {
  ...
  const [substituteId, setSubstituteId] = useState(0);
  ...
  async function load() {
    try {
      const [coverageResult, centerResult, appointmentResult] = await Promise.all([
        api.get<Coverage[]>("/clinical-coverages"), api.get<Center[]>("/centers/mine"), api.get<Appointment[]>("/appointments"),
      ]);
      setCoverages(coverageResult.data); setCenters(centerResult.data); setAppointments(appointmentResult.data);
    } catch (reason: any) { setError(reason?.response?.data?.detail || "No fue posible cargar las coberturas."); }
  }

  useEffect(() => { void load(); }, []);
  useEffect(() => {
    if (!centerId) { setSubstitutes([]); return; }
    void api.get<Substitute[]>("/clinical-coverages/eligible-substitutes", { params: { center_id: centerId } })
      .then(({ data }) => setSubstitutes(data))
      .catch((reason) => setError(reason?.response?.data?.detail || "No fue posible cargar suplentes."));
  }, [centerId]);
  ...
  async function create() {
    ...
    if (endsAt <= startsAt) { setError("La fecha final debe ser posterior al inicio."); return; }
    setSaving(true); setError("");
    try {
      await api.post("/clinical-coverages", { center_id: centerId, substitute_doctor_id: substituteId, starts_at: startsAt, ends_at: endsAt });
      setSubstituteId(0); setStartsAt(""); setEndsAt(""); await load();
    } catch (reason: any) { setError(reason?.response?.data?.detail || "No fue posible crear la cobertura."); }
    finally { setSaving(false); }
  }

  async function revoke(id: number, expired: boolean) {
...
function CoverageList({ title, items, transferableByCoverage, onRevoke, onTransfer }: { title: string; items: Coverage[]; transferableByCoverage?: Map<number, Appointment[]>; onRevoke?: (id: number, expired: boolean) => void; onTransfer?: (coverageId: number, appointmentId: number) => void }) {
  return <div className="rounded-xl border bg-white p-5 shadow-sm"><h3 className="font-semibold">{title}</h3><div className="mt-4 space-y-3">{items.map((item) => <div key={item.id} className="rounded-lg border p-4"><div className="flex justify-between gap-3"><div><p className="font-medium">{item.principal_doctor_name} → {item.substitute_doctor_name}</p><p className="text-sm text-slate-600">{item.center_name}</p><p className="mt-1 text-xs text-slate-500">Desde {new Date(item.starts_at).toLocaleString()}<br />Hasta {new Date(item.ends_at).toLocaleString()}</p></div><span className="text-xs font-semibold">{statusLabels[item.status]}</span></div>
    {item.status === "expired" && <p className="mt-3 rounded bg-amber-50 p-2 text-xs text-amber-800">La autorización terminó. Cierra la cobertura para restaurar las citas pendientes.</p>}
    {onRevoke && item.status !== "revoked" && <button onClick={() => onRevoke(item.id, item.status === "expired")} className="mt-3 text-sm font-medium text-red-700">{item.status === "expired" ? "Cerrar y restaurar citas pendientes" : "Revocar cobertura"}</button>}
    {transferableByCoverage?.get(item.id)?.map((appointment) => <div key={appointment.id} className="mt-3 flex items-center justify-between rounded bg-slate-50 p-2 text-sm"><span>Cita #{appointment.id} · {appointment.patient_name} · {appointment.appointment_date} {appointment.appointment_time.slice(0, 5)}</span><button onClick={() => onTransfer?.(item.id, appointment.id)} className="font-medium text-teal-700">Transferir por cobertura</button></div>)}</div>)}{!items.length && <p className="text-sm text-slate-500">No hay coberturas.</p>}</div></div>;
}
