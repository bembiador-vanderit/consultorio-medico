...
export default function Appointments({ onBack, initialPatient }: Props) {
  const [items, setItems] = useState<Appointment[]>([]);
  const [centers, setCenters] = useState<Center[]>([]);
  const [doctors, setDoctors] = useState<Doctor[]>([]);
  const [patientResults, setPatientResults] = useState<Patient[]>([]);
  const [selectedPatient, setSelectedPatient] = useState<Patient | null>(initialPatient ?? null);
  const [patientQuery, setPatientQuery] = useState(initialPatient ? `${initialPatient.first_name} ${initialPatient.last_name}` : "");
  const [form, setForm] = useState<AppointmentInput>({ ...empty, patient_id: initialPatient?.id ?? 0 });
  const [editing, setEditing] = useState<Appointment | null>(null);
  const [showForm, setShowForm] = useState(Boolean(initialPatient));
  ...
  async function load() {
    ...
    try {
      const [a, c] = await Promise.all([
        ...
        api.get<Center[]>("/centers/mine"),
      ]);
      setItems(a.data);
      setCenters(c.data);
    } catch (e: any) {
      setError(e?.response?.data?.detail || "No fue posible cargar la agenda.");
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    if (!showForm || selectedPatient || patientQuery.trim().length < 2) {
      setPatientResults([]);
      return;
    }
    const timer = window.setTimeout(async () => {
      ...
      try {
        const { data } = await api.get<Patient[]>("/patients", { params: { query: patientQuery.trim(), limit: 10 } });
        setPatientResults(data);
      } catch (e: any) {
        setPatientResults([]);
        setError(e?.response?.data?.detail || "No fue posible buscar pacientes.");
      } finally {
        setSearchingPatients(false);
      }
    }, 250);
    return () => window.clearTimeout(timer);
  }, [patientQuery, selectedPatient, showForm]);

  async function loadDoctors(centerId: number | null, date: string) {
    if (!centerId || !date) { setDoctors([]); return; }
    setLoadingDoctors(true);
    try {
      const { data } = await api.get<Doctor[]>("/appointments/doctors", { params: { center_id: centerId, appointment_date: date } });
      setDoctors(data);
      if (form.doctor_id && !data.some((d) => d.id === form.doctor_id)) setForm((f) => ({ ...f, doctor_id: null }));
    } catch (e: any) {
      setDoctors([]);
      setError(e?.response?.data?.detail || "No fue posible consultar los médicos disponibles.");
  ...
  function openNew() {
    ...
    setForm({ ...empty, center_id: center });
    setShowForm(true);
    if (center) void loadDoctors(center, empty.appointment_date);
  }

  async function edit(a: Appointment) {
    setEditing(a);
    try {
      const { data } = await api.get<Patient>(`/patients/${a.patient_id}`);
      setSelectedPatient(data);
      setPatientQuery(`${data.first_name} ${data.last_name}`);
    } catch {
      setSelectedPatient(null);
      setPatientQuery(a.patient_name);
    }
    setForm({ patient_id: a.patient_id, doctor_id: a.doctor_id, center_id: a.center_id, appointment_date: a.appointment_date, appointment_time: a.appointment_time.slice(0, 5), reason: a.reason || "", status: a.status, notes: a.notes || "" });
    setShowForm(true);
    if (a.center_id) void loadDoctors(a.center_id, a.appointment_date);
  }
  ...
  function clearPatient() {
    ...
    setPatientQuery("");
    setPatientResults([]);
    setForm((f) => ({ ...f, patient_id: 0 }));
  }

  async function save() {
    setError("");
    try {
      if (!form.patient_id) throw new Error("Seleccione un paciente.");
      if (!form.center_id) throw new Error("Seleccione el centro donde se realizará la cita.");
      if (!form.doctor_id) throw new Error("Seleccione el médico que atenderá la cita.");
      if (editing) await api.put(`/appointments/${editing.id}`, form); else await api.post("/appointments", form);
      setShowForm(false);
      setSelectedPatient(null);
      await load();
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || "No fue posible guardar la cita.");
    }
  }

  async function remove(id: number) {
    if (!confirm("¿Eliminar esta cita?")) return;
    try { await api.delete(`/appointments/${id}`); await load(); } catch (e: any) { setError(e?.response?.data?.detail || "No fue posible eliminar la cita."); }
  }
