...
export default function Appointments({ user, onBack, initialPatient, canAccessClinical, onAttendAppointment }: Props) {
  ...
  return <section>
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl"><h3 className="text-xl font-bold">{editing ? "Editar cita" : "Nueva cita"}</h3>
      ...
      <label className="mt-4 block text-sm font-medium">Especialidad{contextLocked ? <div className="mt-1 rounded-lg border bg-slate-100 p-2.5">{editing?.specialty_name || "Especialidad registrada"}</div> : <select value={form.specialty_id || ""} onChange={(e) => setForm({ ...form, specialty_id: Number(e.target.value) || null })} className="mt-1 w-full rounded-lg border p-2.5 disabled:bg-slate-100" required disabled={!form.doctor_id}><option value="">Seleccione...</option>{(doctors.find((doctor) => doctor.id === form.doctor_id)?.specialties ?? []).map((specialty) => <option key={specialty.id} value={specialty.id}>{specialty.name}</option>)}</select>}</label>
