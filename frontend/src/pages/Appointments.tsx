...
export default function Appointments({ user, onBack, initialPatient, canAccessClinical, onAttendAppointment }: Props) {
  ...
  async function edit(a: Appointment) { setFormError(""); setEditing(a); try { const { data } = await api.get<Patient>(`/patients/${a.patient_id}`); setSelectedPatient(data); setPatientQuery(`${data.first_name} ${data.last_name}`); } catch { setSelectedPatient(null); setPatientQuery(a.patient_name); } setForm({ patient_id: a.patient_id, doctor_id: a.doctor_id, center_id: a.center_id, specialty_id: a.specialty_id, appointment_date: a.appointment_date, appointment_time: a.appointment_time.slice(0, 5), reason: a.reason || "", status: a.status, notes: a.notes || "" }); setShowForm(true); if (a.center_id) void loadDoctors(a.center_id, a.appointment_date, a.doctor_id); }
  ...
  function clearPatient() { if (contextLocked) return; setSelectedPatient(null); setPatientQuery(""); setPatientResults([]); setForm((f) => ({ ...f, patient_id: 0 })); }
