...
export default function Appointments({ user, onBack, initialPatient, canAccessClinical, onAttendAppointment }: Props) {
  ...
  return <section>
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl"><h3 className="text-xl font-bold">{editing ? "Editar cita" : "Nueva cita"}</h3>
      ...
      <label className="mt-4 block text-sm font-medium">Centro de atención<select value={form.center_id || ""} disabled={contextLocked} onChange={(e) => { const id = Number(e.target.value) || null; setFormError(""); setForm((current) => ({ ...current, center_id: id, doctor_id: null })); void loadDoctors(id, form.appointment_date, null); }} className="mt-1 w-full rounded-lg border p-2.5 disabled:bg-slate-100" required><option value="">Seleccione...</option>{centers.map((c) => <option key={c.id} value={c.id}>{c.name} — {c.city}</option>)}</select></label>
      ...
      <label className="mt-4 block text-sm font-medium">Médico responsable<select value={form.doctor_id || ""} onChange={(e) => setForm({ ...form, doctor_id: Number(e.target.value) || null })} className="mt-1 w-full rounded-lg border p-2.5 disabled:bg-slate-100" required disabled={contextLocked || !form.center_id || loadingDoctors}><option value="">{loadingDoctors ? "Consultando disponibilidad..." : "Seleccione..."}</option>{doctors.map((d) => <option key={d.id} value={d.id}>{d.full_name}</option>)}</select>{contextLocked && <span className="mt-1 block text-xs text-slate-500">La reasignación solo se realiza mediante una cobertura clínica explícita.</span>}</label>
