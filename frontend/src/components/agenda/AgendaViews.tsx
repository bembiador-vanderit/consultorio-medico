    ...
    <section aria-labelledby="agenda-week-heading">
      <header className="agenda-view-heading">
        <div>
          ...
          <p className="atlas-muted">Una sola carga para los siete días.</p>
        </div>
      </header>
      <div className="agenda-week-grid">
        {Array.from({ length: 7 }, (_, index) => {
          ...
          const entries = appointments.filter(
            (item) => item.appointment_date === day,
          );
          return (
            <Card key={day} className="agenda-week-day">
              <h3>{formatDate(day, { weekday: "short", day: "numeric" })}</h3>
              {entries.length ? (
                sortByTime(entries).map((item) => (
                  <AppointmentCard
                    key={item.id}
                    appointment={item}
                    onSelect={onSelect}
                ...
                <p className="atlas-help">Sin citas</p>
              )}
            </Card>
          );
        })}
