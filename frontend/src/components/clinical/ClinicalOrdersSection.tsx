...
import { clinicalApi, isReadAborted } from "../../services/clinicalApi";
import type { LaboratoryOrder, LaboratoryTest, MedicalStudy, StudyOrder } from "../../types/clinicalOrder";

type Props = { historyId: number; specialtyId: number | null; completed: boolean; allowAdditional?: boolean; onOrdersChanged?: () => void | Promise<void>; onDirtyChange?: (dirty: boolean) => void };
...
export default function ClinicalOrdersSection({ historyId, specialtyId, completed, allowAdditional = false, onOrdersChanged, onDirtyChange }: Props) {
  ...
  async function load(generation = generationRef.current, signal?: AbortSignal) {
    try {
      const [testResponse, studyResponse, laboratoryResponse, studyOrderResponse] = await Promise.all([
        clinicalApi.getLaboratoryCatalog(),
        clinicalApi.getStudyCatalog(specialtyId, true),
        clinicalApi.getLaboratoryOrders(historyId, { signal }),
        clinicalApi.getStudyOrders(historyId, { signal }),
      ]);
      if (!isCurrent(generation)) return;
      setTests(testResponse); setStudies(studyResponse); setLaboratoryOrders(laboratoryResponse); setStudyOrders(studyOrderResponse);
    } catch (reason: any) { if (isCurrent(generation) && !isReadAborted(reason)) setError(reason?.response?.data?.detail || "No fue posible cargar las órdenes clínicas."); }
  }
  ...
  useEffect(() => {
    ...
    return () => { controller.abort(); if (readControllerRef.current === controller) readControllerRef.current = null; if (generationRef.current === generation) generationRef.current += 1; };
  }, [historyId, specialtyId]);

  useEffect(() => { onDirtyChange?.(composer !== null); }, [composer, onDirtyChange]);
  ...
  return <section className="min-w-0 rounded-xl border bg-white p-4 shadow-sm sm:p-5" aria-busy={Boolean(busy)}>
    ...
    {error && <div role="alert" className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div>}{message && <div role="status" aria-live="polite" className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">{message}</div>}
  </section>;
}
