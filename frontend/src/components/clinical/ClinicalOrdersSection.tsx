...
export default function ClinicalOrdersSection({ historyId, specialtyId, completed }: Props) {
  ...
  async function saveLaboratoryOrder() {
    ...
    try {
      ...
      else await api.post(`/clinical-history/${historyId}/laboratory-orders`, payload);
  ...
  async function saveStudyOrder() {
    ...
    try {
      ...
      else await api.post(`/clinical-history/${historyId}/study-orders`, payload);
  ...
  return <div className="space-y-6">
    <section className="rounded-xl border border-cyan-200 bg-cyan-50/40 p-5">
      ...
      {!completed && <div className="mt-4 space-y-4">
      ...
      <div className="mt-5 space-y-3">{laboratoryOrders.map((order) => <article key={order.id} className="rounded-lg border bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-semibold">Orden de laboratorio #{order.id}</p><p className="text-xs text-slate-500">{formatDateTime(order.created_at)} · {order.doctor_name}</p></div><div className="flex gap-2">{!completed && <button type="button" onClick={() => editLaboratoryOrder(order)} className="text-sm font-medium text-cyan-700">Editar</button>}<button type="button" onClick={() => void download("laboratory", order.id)} disabled={Boolean(busy)} className="rounded-md bg-cyan-700 px-3 py-1.5 text-sm font-medium text-white disabled:opacity-40">Imprimir PDF</button></div></div><ul className="mt-3 list-disc pl-5 text-sm">{order.items.map((item) => <li key={item.id}>{item.test_name}{item.custom_note ? ` — ${item.custom_note}` : ""}</li>)}</ul>{order.notes && <p className="mt-2 text-sm"><strong>Observaciones:</strong> {order.notes}</p>}</article>)}{!laboratoryOrders.length && <p className="text-sm text-slate-500">Sin órdenes de laboratorio.</p>}</div>
    </section>
