...
type Props = { value: PatientDemographics; onChange: (value: PatientDemographics) => void; section: "personal" | "clinical" };
...
export default function PatientDemographicFields({ value, onChange, section }: Props) {
  ...
  useEffect(() => {
    ...
    let active = true;
    api.get<Locality[]>("/patients/localities").then(({ data }) => { if (active) setLocalities(data); }).catch(() => { if (active) setLocalityError(true); });
    return () => { active = false; };
  }, [section]);
  const change = (key: keyof PatientDemographics, next: string) => onChange({ ...value, [key]: next || null });
  const input = (key: keyof PatientDemographics, label: string, maxLength = 100, type = "text") => <FormField key={key} label={label}><Input type={type} maxLength={maxLength} value={value[key] ?? ""} onChange={(event) => change(key, event.target.value)} /></FormField>;
  if (section === "clinical") return <>
    <FormSection title="Datos clínicos registrados" description="Datos de la ficha del paciente. Los antecedentes, diagnósticos y notas se registran en la historia clínica.">
      <FormField label="Sexo registrado para fines clínicos"><Select value={value.registered_sex ?? ""} onChange={(event) => change("registered_sex", event.target.value)}><option value="">Sin registrar</option>{Object.entries(sexLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</Select></FormField>
      <FormField label="Tipo sanguíneo" description="Declarado/registrado; no equivale a confirmación de laboratorio."><Select value={value.blood_type ?? ""} onChange={(event) => change("blood_type", event.target.value)}><option value="">Desconocido / sin registrar</option>{bloodTypes.map((type) => <option key={type}>{type}</option>)}</Select></FormField>
    </FormSection>
  </>;
  return <>
    <FormSection title="Documento y domicilio">
      <FormField label="Tipo de documento"><Select value={value.document_type ?? ""} onChange={(event) => onChange({ ...value, document_type: event.target.value || null, ...(!event.target.value ? { document_number: null } : {}) })}><option value="">Sin documento</option>{Object.entries(documentLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</Select></FormField>
      ...
      <FormField label="Municipio / localidad"><Select disabled={localityError} value={value.locality_id ?? ""} onChange={(event) => onChange({ ...value, locality_id: event.target.value ? Number(event.target.value) : null })}><option value="">Sin registrar</option>{value.locality_id && !localities.some((item) => item.id === value.locality_id) && <option value={value.locality_id}>Localidad registrada (conservar)</option>}{localities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></FormField>
