...
export type AppView = "finance" | "insurance" | "security" | "dashboard" | "patients" | "appointments" | "reports" | "care-context" | "availability" | "users" | "follow-ups" | "consultation" | "clinical-coverages";
...
export const navigationItems: readonly NavigationItem[] = [
  { id: "finance", label: "Finanzas", view: "finance", icon: "report", group: "administration", order: 92, visibility: { kind: "authenticated" } },
...
export const roleNavigationVisibility: NavigationVisibilityResolver = (item, user) => {
  if (item.view === "finance") return !!user.permissions?.some(permission => permission === "finance:read" || permission === "ars:read");
