...
type View = "dashboard" | "patients" | "appointments" | "care-context" | "availability" | "users";
...
function App() {
  ...
  async function signIn(event: FormEvent) {
    ...
    try {
      ...
      const profile = await api.get<User>("/auth/me"); setUser(profile.data); setPassword("");
    } catch (reason) {
      console.error(reason); setAccessToken(null);
  ...
  async function signOut() { await logoutSession(); setUser(null); setView("dashboard"); setSelectedAppointmentPatient(null); }
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900"><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-teal-700">Consultorio Médico</p><h1 className="text-lg font-bold">Sistema de gestión</h1></div><div className="flex items-center gap-4"><div className="hidden text-right sm:block"><p className="text-sm font-semibold">{user.full_name}</p><p className="text-xs text-slate-500">{user.roles.join(", ")}</p></div><button onClick={() => void signOut()} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">Cerrar sesión</button></div></div></header><div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-6 md:flex-row"><aside className="w-full rounded-xl border bg-white p-3 shadow-sm md:w-56 md:self-start"><button onClick={() => setView("dashboard")} className={`w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "dashboard" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>🏠 Dashboard</button><button onClick={() => { setSelectedAppointmentPatient(null); setView("appointments"); }} className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "appointments" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>📅 Agenda</button><button onClick={() => setView("patients")} className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "patients" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>👤 Pacientes</button>{isDoctor && <button onClick={() => setView("availability")} className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "availability" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>🗓️ Mi disponibilidad</button>}{isAdmin && <><button onClick={() => setView("users")} className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "users" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>👥 Usuarios y roles</button><button onClick={() => setView("care-context")} className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${view === "care-context" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"}`}>🏥 Localidades y centros</button></>}</aside><section className="min-w-0 flex-1">{view === "patients" ? <Patients user={user} onBack={() => setView("dashboard")} onPatientChanged={() => setPatientsVersion((value) => value + 1)} onScheduleAppointment={schedulePatient} /> : view === "appointments" ? <Appointments onBack={() => setView("dashboard")} initialPatient={selectedAppointmentPatient} /> : view === "care-context" ? <CareContext onBack={() => setView("dashboard")} /> : view === "availability" ? <DoctorAvailability onBack={() => setView("dashboard")} /> : view === "users" ? <Users onBack={() => setView("dashboard")} /> : <Dashboard user={user} patientsVersion={patientsVersion} />}</section></div></main>;
}
