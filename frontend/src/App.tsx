...
function App() {
  ...
  async function signIn(event: FormEvent) {
    ...
    try {
      ...
      setAccessToken(null);
      setError(
        reason instanceof AxiosError && reason.response?.status === 401
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-100 p-6 text-slate-900">
        <form onSubmit={signIn} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg">
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-6 w-full rounded-lg bg-teal-700 p-2.5 font-medium text-white"
          >
        ...
        <aside className="w-full rounded-xl border bg-white p-3 shadow-sm md:w-56 md:self-start">
          <button
            ...
            className={`w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${
              view === "dashboard" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"
            }`}
          >
            🏠 Dashboard
          </button>
          ...
          <button
            ...
            className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${
              view === "appointments" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"
            }`}
          >
            📅 Agenda
          </button>
          ...
          <button
            ...
            className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${
              view === "patients" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"
            }`}
          >
            👤 Pacientes
          </button>
          ...
          {isDoctor && (
            <button
              ...
              className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${
                view === "availability" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"
              }`}
            >
              🗓️ Mi disponibilidad
            </button>
          ...
          {isAdmin && (
            <button
              onClick={() => setView("care-context")}
              className={`mt-1 w-full rounded-lg px-3 py-2 text-left text-sm font-medium ${
                view === "care-context" ? "bg-teal-50 text-teal-800" : "hover:bg-slate-50"
              }`}
            >
        ...
        <section className="min-w-0 flex-1">
          {view === "patients" ? (
            <Patients
              ...
              onPatientChanged={() => setPatientsVersion((value) => value + 1)}
            />
          ) : view === "appointments" ? (
            <Appointments onBack={() => setView("dashboard")} />
          ) : view === "care-context" ? (
            <CareContext onBack={() => setView("dashboard")} />
          ) : view === "availability" ? (
            <DoctorAvailability onBack={() => setView("dashboard")} />
          ) : (
