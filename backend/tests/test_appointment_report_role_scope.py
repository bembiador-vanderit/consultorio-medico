...
def scoped_db():
    ...
    with Session(engine) as db:
        ...
        db.add_all([admin, doctor_one, doctor_two, doctor_three, doctor_four, doctor_five, secretary, secretary_all, outsider, inactive, patient])
        db.flush()
        db.add_all([
            SecretaryCenterScope(secretary_id=secretary.id, center_id=center_one.id, manage_all_doctors=False, doctors=[doctor_one, doctor_two]),
            ...
            Appointment(patient_id=patient.id, doctor_id=doctor_four.id, center_id=center_one.id, appointment_date=date(2026, 9, 12), appointment_time=time(12, 0)),
            Appointment(patient_id=patient.id, doctor_id=doctor_five.id, center_id=center_three.id, appointment_date=date(2026, 9, 13), appointment_time=time(13, 0)),
        ])
        db.commit()
...
def test_doctor_only_sees_own_agenda_and_report(scoped_db):
    ...
    options = appointment_scope_options(user=doctor_one, db=db)
    assert [doctor["id"] for doctor in options["doctors"]] == [doctor_one.id]
...
def test_secretary_specific_scope_spans_centers_and_is_not_creator_based(scoped_db):
    ...
    unauthorized = db.scalar(select(User).where(User.email == "four@example.com"))
    other_center = db.scalar(select(User).where(User.email == "five@example.com"))
    items = list_appointments(user=secretary, db=db)
    assert {item.doctor_id for item in items} == {doctor_one.id, doctor_two.id, doctor_three.id}
    assert unauthorized.id not in {item.doctor_id for item in items}
    assert other_center.id not in {item.doctor_id for item in items}
...
def test_secretary_all_doctors_scope_is_limited_to_its_center(scoped_db):
    ...
    same_center = db.scalar(select(User).where(User.email == "four@example.com"))
    items = list_appointments(user=secretary_all, db=db)
    assert {item.doctor_id for item in items} == {doctor_one.id, doctor_two.id, same_center.id}
...
def test_manipulated_filters_cannot_escape_secretary_scope(scoped_db):
    db, _, _, _, doctor_three, secretary, *_, center_one, center_two = scoped_db
    unauthorized = db.scalar(select(User).where(User.email == "four@example.com"))
    assert list_appointments(center_id=center_one.id, doctor_id=unauthorized.id, user=secretary, db=db) == []
...
def test_secretary_new_appointment_doctor_options_and_validation_are_scoped(scoped_db):
    ...
    assert {doctor["id"] for doctor in available} == {doctor_one.id, doctor_two.id}
    with pytest.raises(HTTPException) as error:
        validate_appointment_assignment(db, secretary, unauthorized.id, center_one.id, date(2026, 9, 10))
...
def test_scope_options_do_not_require_admin_catalog_endpoints(scoped_db):
    ...
    assert {doctor["id"] for doctor in options["doctors"]} == {doctor_one.id, doctor_two.id, doctor_three.id}
...
def test_pdf_and_report_delivery_use_the_same_server_side_scope(scoped_db, monkeypatch):
    ...
    unauthorized = db.scalar(select(User).where(User.email == "four@example.com"))
    assert set(captured) == {doctor_one.id, doctor_two.id, doctor_three.id}
    assert unauthorized.id not in captured
...
def test_doctor_pdf_contains_only_own_appointments(scoped_db, monkeypatch):
    db, _, doctor_one, doctor_two, *_ = scoped_db
    captured = []
