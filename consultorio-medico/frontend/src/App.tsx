...
function App() {
  ...
  async function signIn(event: FormEvent) {
    ...
    try {
      ...
      setUser(profile.data); setPassword("");
    } catch (reason) {
  ...
  if (!user) return (
    <main className="flex min-h-screen items-center justify-center bg-slate-100 p-6 text-slate-900">
      <form onSubmit={signIn} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg">
        ...
        <label className="mt-6 block text-sm font-medium">Correo<input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 p-2.5" /></label>
        <label className="mt-4 block text-sm font-medium">Contraseña<input type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 p-2.5" /></label>
        <button type="submit" disabled={loading} className="mt-6 w-full rounded-lg bg-teal-700 p-2.5 font-medium text-white hover:bg-teal-800 disabled:opacity-60">{loading ? "Ingresando..." : "Ingresar"}</button>
      </form>
    </main>
  );
    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-6 md:flex-row">
        ...
        <section className="min-w-0 flex-1">
          {view === "patients" ? <Patients onBack={() => setView("dashboard")} onPatientChanged={() => setPatientsVersion((value) => value + 1)} /> : <Dashboard user={user} patientsVersion={patientsVersion} />}
        </section>
      </div>
    </main>
  );
...
function Dashboard({ user, patientsVersion }: { user: User; patientsVersion: number }) {
  ...
  useEffect(() => { loadPatientCount(); }, [patientsVersion]);
  ...
  return <>
    ...
    <div className="mt-6 rounded-xl border bg-white p-6 shadow-sm"><h3 className="font-bold">Acciones rápidas</h3><p className="mt-1 text-sm text-slate-500">Utilice el menú para acceder a los módulos del sistema.</p></div>
  </>;
}

function StatCard({ title, value }: { title: string; value: string | number }) {
  return <div className="rounded-xl border bg-white p-6 shadow-sm"><p className="text-sm text-slate-500">{title}</p><p className="mt-2 text-3xl font-bold">{value}</p></div>;
}
